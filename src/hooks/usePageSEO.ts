...
import { LanguageCode, LANGUAGES, getBcp47Locale } from "@/context/LanguageContext";
...
// ISO 639-1 language codes for Google Search Console hreflang mapping
export const HREFLANG_MAP: Record<LanguageCode, string> = {
  en: "en",
  jp: "ja", // Google requires 'ja' for Japanese
  "zh-Hant": "zh-Hant",
  "zh-Hans": "zh-Hans",
  fr: "fr",
  es: "es",
...
const VIEW_TITLES: Record<LanguageCode, Record<ViewRoute, string>> = {
  en: {
...
const VIEW_DESCRIPTIONS: Record<LanguageCode, Record<ViewRoute, string>> = {
  en: {
...
const BASE_URL = "https://ebookcc.com";

/**
 * Builds the canonical URL for a given view and language.
 */
export function buildCanonicalUrl(language: LanguageCode, view: ViewRoute): string {
  const viewPart = view === "home" ? "" : `/${view}`;
  return `${BASE_URL}/${language}${viewPart}`;
}

/**
 * Builds the root/default URL for a given view.
 */
export function buildRootUrl(view: ViewRoute): string {
  const viewPart = view === "home" ? "" : `/${view}`;
  return `${BASE_URL}${viewPart}`;
}

/**
 * React hook to synchronize all SEO elements in <head>:
...
export function usePageSEO({
  ...
  useEffect(() => {
    ...
    const langDictTitles = VIEW_TITLES[language] || VIEW_TITLES.en;
    const langDictDescs = VIEW_DESCRIPTIONS[language] || VIEW_DESCRIPTIONS.en;

    const title = customTitle || langDictTitles[view] || VIEW_TITLES.en[view];
    const description = customDescription || langDictDescs[view] || VIEW_DESCRIPTIONS.en[view];
    const canonicalUrl = buildCanonicalUrl(language, view);
    ...
    setMetaTag("property", "og:locale", getBcp47Locale(language).replace("-", "_"));
    ...
    // 7. Canonical Tag Management (Ensures single, accurate canonical link)
    ...
    // Remove existing hreflang alternate links to prevent stale or duplicate links
    ...
    // Add x-default fallback (root clean view)
    const xDefaultLink = document.createElement("link");
    xDefaultLink.setAttribute("rel", "alternate");
    xDefaultLink.setAttribute("hreflang", "x-default");
    xDefaultLink.setAttribute("href", buildRootUrl(view));
    document.head.appendChild(xDefaultLink);

    // Add all 12 language-specific hreflang alternate links
    LANGUAGES.forEach((lang) => {
      const hreflangCode = HREFLANG_MAP[lang.code] || lang.code;
      const altLink = document.createElement("link");
      altLink.setAttribute("rel", "alternate");
      altLink.setAttribute("hreflang", hreflangCode);
      altLink.setAttribute("href", buildCanonicalUrl(lang.code, view));
